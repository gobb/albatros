...
<I> 
 ...
 <table border="0">
  <th>Döviz Kodu<br/><i>Currency Code</i></th> 
  <th>Döviz Adı<br/><i>Currency Name</i></th> 
  <th>Döviz Alış<br/><i>Forex Buying</i></th> 
  <th>Döviz Satış<br/><i>Forex Selling</i></th> 
  <th>Efektif Alış<br/><i>Banknote Buying</i></th> 
  <th>Efektif Satış<br/><i>Banknote Selling</i></th> 
 ...
 <td class="para">
  ...
  <i><xsl:value-of select="CurrencyName" /></i> 
  </td>
 <td class="deger">
...
<table>
 <th colspan="4">Çapraz Kurlar<br/><i>Cross Rates</i></th>
<xsl:for-each select="Tarih_Date/Currency">

<xsl:if test="CrossRateUSD&gt;'0'">
<xsl:if test="@Kod!='USD'">
	<tr>
	<td class="para"><xsl:value-of select="@Kod" />/USD</td>
	<td class="para">1 AMERIKAN DOLARI</td>
	<td class="deger"><xsl:value-of select="CrossRateUSD" /></td>
	<td class="deger"><xsl:value-of select="Isim" /></td> 
	</tr>
</xsl:if>
</xsl:if>
...
<xsl:if test="CrossRateOther&gt;'0'">
	<tr>
	<td class="para">USD/<xsl:value-of select="@Kod" /></td>
	<td class="para">1 <xsl:value-of select="Isim" /></td>
	<td class="deger"><xsl:value-of select="CrossRateOther" /></td>
...
<table>
<tr><th colspan="4">Euro Dönüşüm Kurları;</th></tr>
<xsl:for-each select="Tarih_Date/Currency">
<xsl:if test="CrossRateEuro&gt;='0'">
	<tr>
	<td class="para"><xsl:value-of select="@Kod" />/EUR</td>
	<td class="para">1 EURO</td>
	<td class="deger"><xsl:value-of select="CrossRateEuro" /></td>
	<td class="deger"><xsl:value-of select="Isim" /></td> 
	</tr>
</xsl:if>
</xsl:for-each>
...
<xsl:if test="@Kod='XDR'">
	...
	<td class="para">1<xsl:value-of select="Isim" />(SDR)</td>
	<td class="deger"><xsl:value-of select="ForexBuying" /></td>
