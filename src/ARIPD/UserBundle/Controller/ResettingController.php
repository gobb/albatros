...
class ResettingController extends BaseController {
	...
	public function requestAction() {
		return $this->container->get('templating')
				->renderResponse(
						'::ARIPDUserBundle/Resetting/request.html.'
								. $this->getEngine());
	}
	...
	public function sendEmailAction(Request $request) {
		...
		if ($user
			...
			return $this->container->get('templating')
					->renderResponse(
							'::ARIPDUserBundle/Resetting/passwordAlreadyRequested.html.'
									. $this->getEngine());
		}

		if (null === $user->getConfirmationToken()) {
			/** @var $tokenGenerator \FOS\UserBundle\Util\TokenGeneratorInterface */
			$tokenGenerator = $this->container
		...
		$this->container->get('aripduser.log_service')
				->saveLog($user, 'logtype___user_resetting_request');

		return new RedirectResponse(
				$this->container->get('router')
						->generate('fos_user_resetting_check_email'));
	}
	...
	public function checkEmailAction() {
		...
		return $this->container->get('templating')
				->renderResponse(
						'::ARIPDUserBundle/Resetting/checkEmail.html.'
								. $this->getEngine(),
						array('email' => $email,));
	}
	...
	public function resetAction(Request $request, $token) {
		...
		$dispatcher
				->dispatch(FOSUserEvents::RESETTING_RESET_INITIALIZE, $event);
		...
		if ('POST' === $request->getMethod()) {
			...
			if ($form->isValid()) {
				...
				$dispatcher
						->dispatch(FOSUserEvents::RESETTING_RESET_SUCCESS,
								$event);
				...
				$dispatcher
						->dispatch(FOSUserEvents::RESETTING_RESET_COMPLETED,
				 ...
				 *****************************************************/
				$this->container->get('aripduser.log_service')
						->saveLog($user, 'logtype___user_resetting_confirmed');
		...
		return $this->container->get('templating')
				->renderResponse(
						'::ARIPDUserBundle/Resetting/reset.html.'
								. $this->getEngine(),
						array('token' => $token, 'form' => $form->createView(),));
	}
